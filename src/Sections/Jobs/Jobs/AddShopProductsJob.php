...
use NetLinker\AutoDelivererStore\Sections\Tasks\Services\Jobs;
...
class AddShopProductsJob implements ShouldQueue
{
    ...
    public function __construct($params = [])
    {
        ...
        $this->prepareStatus(['name' => __('deliverer-agrip::jobs.add_shop_products', ['name' => config('deliverer-agrip.name')])]);
    ...
    public function handle()
    {
        ...
        $service = new AddShopProducts($this->params['shop_uuid'], $this->params['owner_uuid'], $this->params['configuration_uuid'],
            function ($message) {
                $this->addLog($message);
            }
        );

        $steps = $service->addShopProducts();
