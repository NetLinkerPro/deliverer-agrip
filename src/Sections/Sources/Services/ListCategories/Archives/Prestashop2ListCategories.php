...
class Prestashop2ListCategories implements ListCategories
{
...
//        $categories = $this->getMainCategories();
    ...
    private function getMainCategoriesWebsite(): array
    {
        ...
        $categories = [];
        $crawler->filter('#mega-menu-top_nav > li > a')->each(function (Crawler $aElement) use (&$categories) {
            $name = $this->getTextCrawler($aElement);
            $id = sprintf('0-%s', Str::slug($name));
    ...
    {
        if (!$crawlerElements) {
            $crawlerElements = $this->getCrawlerCategories()->filter('#cbp-hrmenu > ul > li.cbp-has-submeu');
        }
        $categories = [];
        $crawlerElements->each(function (Crawler $element) use (&$categories, &$depth, &$parentCategory) {
            ...
            if ($this->liHasChild($element)) {
                $nextDepth = $depth + 1;
                if ($nextDepth === 1){
                    $crawlerChildElements = $element->filter('div.col-12 > div.cbp-category-link-w');
                } elseif ($nextDepth === 2){
                    $crawlerChildElements = $element->filter('ul li');
                } else {
                    return;
                }
    ...
    {
        $content = $this->websiteClient->getContents('https://b2b.agrip.pl');
    ...
    {
        $keyCache = 'deliverer-agrip_main_categories_2';
