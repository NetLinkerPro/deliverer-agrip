...
class PhpListCategories implements ListCategories
{
    use CrawlerHtml;
...
//        $categories = $this->getCategories();
    ...
    private function getMainCategoriesWebsite(): array
    {
        ...
        $categories = [];
        $crawler->filter('#jstree > ul > li > a')->each(function (Crawler $aElement) use (&$categories) {
            $name = explode(' [', $this->getTextCrawler($aElement))[0];
