...
class Ekspert2ListProducts implements ListProducts
{
    ...
    private function getProduct(Crawler $containerProduct): ?ProductSource
    {
        ...
        $availability = 1;
        $url = sprintf('https://b2b.ama-europe.pl%s', $this->getAttributeCrawler($containerProduct->filter('.name h2 a'), 'href'));
    ...
    private function getCrawlerPage(int $page): Crawler
    {
        ...
        $url = sprintf('https://b2b.ama-europe.pl/offer/index/page/%s?controller=offer&action=index&module=default&search=&field%%5B0%%5D=wszystko&producer=&quantity=s&category=', $page);
    ...
    {
        $name = $this->getTextCrawler($containerProduct->filter('div.name h2 a span'));
        if (Str::contains(mb_strtolower($name), '(na zam.')){
            return 0;
        }
        $textStock =  $this->getTextCrawler($containerProduct->filter('span.label_status_available'));
        if ($textStock === 'dostępny'){
            return 5;
        }
        return 0;
    }
