...
class XmlPlatformListProducts implements ListProducts
{
    ...
    {
        $categories = [];
        $breadcrumbs = $this->getStringXml($xmlProduct->kategoria);
...
//            $descriptionProduct = str_replace('<br /><br />', '<br />', $descriptionProduct);
    ...
    {
        return (float) $this->getStringXml($xmlProduct->cena);
    }
    ...
    {
        return (int) $this->getStringXml($xmlProduct->ilosc);
    }
     ...
     * @throws DelivererAgripException
     */
    private function getId($xmlProduct): string
    {
        $id = $this->getStringXml($xmlProduct->model);
        if (!$id){
