...
class XmlPrestashopListProducts implements ListProducts
{
    ...
    {
        $this->url = 'https://b2b.agrip.pl/availability/PLN_pl.xml';
    }
    ...
    private function getXmlProducts(): Generator
    {
        ...
        foreach ($xmlProducts as $xmlProduct) {
            $id = str_replace(',', '-', $this->getStringXml($xmlProduct['id']));
...
//            $descriptionProduct = str_replace('<br /><br />', '<br />', $descriptionProduct);
    ...
    {
        $available = (int) $this->getStringXml($xmlProduct->available);
        if ($available){
            return 5;
        }
        return 0;
