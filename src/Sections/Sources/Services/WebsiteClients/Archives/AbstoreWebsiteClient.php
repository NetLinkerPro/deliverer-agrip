...
class AbstoreWebsiteClient implements WebsiteClient
{
     ...
     */
    public function getContents(string $url, array $options = [], int $attempt = 2, string $contentValid = '<a href="/client/account/index/"'): string
    {
    ...
    private function getCookiesLogin(bool $forceLogin = false): CookieJar
    {
        ...
        return Cache::remember($keyCache, 3600, function () {
            $client = $this->getClientAnonymous();
            $response = $client->post('https://agrip.abstore.pl/client/loginorcreate/login/', $this->getDefaultOptions([
            ...
            $contents = $response->getBody()->getContents();
            if (!Str::contains($contents, '<a href="/client/account/index/" ')) {
     ...
     */
    public function getContentAjax(string $url, array $options = [], string $method = 'POST', string $contentValid = '{"code":"200","messages":', int $attempts = 2): string
    {
