...
class IdosellWebsiteClient implements WebsiteClient
{
    use CrawlerHtml;
    ...
    private function getCookiesLogin(): CookieJar
    {
        ...
        return Cache::remember($keyCache, 3600, function () {
            ...
            $response = $client->post('https://agrip.pl/signin.php', $optionsPost);
            $contents = $response->getBody()->getContents();
            if (!$this->isLoggedClient($contents)) {
     ...
     */
    public function getContentAjax(string $url, array $options = [], string $method = 'POST', string $contentValid = '{"userId":'): string
    {
    ...
    {
        return Str::contains($contents, '{"userId":');
    }
