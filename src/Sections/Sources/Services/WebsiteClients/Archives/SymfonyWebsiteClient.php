...
class SymfonyWebsiteClient implements WebsiteClient
{
    use CrawlerHtml;
    ...
    public function getContents(string $url, array $options = []): string
    {
        ...
        if (!Str::contains($content, '<a class="user" href="/logout">wyloguj</a>')){
    ...
    private function getCookiesLogin(): CookieJar
    {
        ...
        return Cache::remember($keyCache, 3600, function(){
            $client = $this->getClientAnonymous();
            $response = $client->post('https://agrip.de/login', [
                RequestOptions::FORM_PARAMS => [
                    'signin[username]' => $this->login,
