...
class ComarchWebsiteClient implements WebsiteClient
{
    use CrawlerHtml;
        ...
        return Cache::remember($keyCache, 3600, function () {
            ...
            $options = $this->getDefaultOptionsRequestAjax([
                'headers' => [
                    ...
                    'Referer' => 'https://agrip.pl/logowanie,7',
                ],
            ...
            $response = $client->post('https://agrip.pl/logowanie,7', $options);
            $contents = $response->getBody()->getContents();
            $responseJson = json_decode($contents, true);
            $logged = $responseJson['action']['Result'] ?? false;
            if (!$logged) {
                throw new DelivererAgripException('Failed login to Agrip');
            }
            $response = $client->get('https://agrip.pl');
            $contents = $response->getBody()->getContents();
