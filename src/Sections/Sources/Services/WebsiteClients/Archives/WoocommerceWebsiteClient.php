...
class WoocommerceWebsiteClient implements WebsiteClient
{
    use CrawlerHtml;
    ...
    public function getContents(string $url, array $options = [], int $attempts = 2): string
    {
        ...
        if (!Str::contains($contents, "'logged_in': 'yes'")) {
            if ($attempts > 1) {
                $attempts -= 1;
                sleep(5);
    ...
    private function getCookiesLogin(bool $forceLogin = false): CookieJar
    {
        ...
        return Cache::remember($keyCache, 3600, function () {
            $client = $this->getClientAnonymous();
            $nonceKey = $this->getNonceKey($client);
            $response = $client->post('https://www.agrip.pl/moje-konto/', [
                RequestOptions::FORM_PARAMS => [
                    'username' => $this->login,
                    'password' => $this->password,
                    'woocommerce-login-nonce' => $nonceKey,
                    '_wp_http_referer' => '/moje-konto/',
                    'login' => 'Zaloguj się',
                ],
            ]);
            $content = $response->getBody()->getContents();
            if (!Str::contains($content, "'logged_in': 'yes'")) {
     ...
     * @return string
     */
    private function getNonceKey(Client $client): string
    {
        $response = $client->get('https://www.agrip.pl/moje-konto/');
        $contents = $response->getBody()->getContents();
        $crawler = $this->getCrawler($contents);
        return $this->getAttributeCrawler($crawler->filter('input[name="woocommerce-login-nonce"]'), 'value');
    }
}