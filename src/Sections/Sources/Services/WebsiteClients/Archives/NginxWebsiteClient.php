...
class NginxWebsiteClient implements WebsiteClient
{
    use CrawlerHtml;
    ...
    public function getContents(string $url, array $options = []): string
    {
        ...
        $contents = iconv("windows-1250", "utf-8//TRANSLIT//IGNORE", $contents);
     ...
     * @throws DelivererAgripException
     */
    public function postContent(string $url, array $options = []): string
    {
        $client = $this->getClient();
        DelivererLogger::log(sprintf('Get content %s', $url));
        ...
        return Cache::remember($keyCache, 3600, function () {
           ...
           $optionsPost = $this->getDefaultOptionsRequest([
                RequestOptions::FORM_PARAMS => [
                    'logowanie[0]' => $this->login,
                    'logowanie[1]' => $this->password,
                    'x' => '43',
                    'y' => '11',
                    'logowanie[2]' =>'1',
                ],
            ]);
            $response = $client->post('https://b2b.agrip.pl/start.html', $optionsPost);
            $contents = $response->getBody()->getContents();
            if (!$this->isLoggedClient($contents)) {
    ...
    {
        return Str::contains($contents, '<td class="opis">Osoba zalogowana:</td>');
    }
