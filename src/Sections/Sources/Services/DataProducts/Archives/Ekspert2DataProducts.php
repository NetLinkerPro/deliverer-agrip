...
class Ekspert2DataProducts implements DataProducts
{
    ...
    /** @var Ekspert2WebsiteClient $webapiClient */
    ...
    {
        $this->websiteClient = app(Ekspert2WebsiteClient::class, [
            'login' => $login,
            'password' => $password,
        ]);
    }
...
//        }
        $ean = $this->getTextCrawler($crawlerProduct->filter('a.ean'));
        $ean = explode(',', $ean)[0] ?? '';
        if ($ean) {
            $product->addAttribute('EAN', $ean, 100);
        }
        $sku =$this->getTextCrawler($crawlerProduct->filter('div.indeks'));
        $sku = trim(str_replace('Indeks:', '',$sku));
        if ($sku) {
...
//        $this->addAttributesFromDescriptionProduct($product, $crawlerProduct);
    ...
    {
            $crawlerProduct->filter('#productInfo .fotoContainer a')
                ->each(function(Crawler $aElement) use (&$product){
                    $href = $this->getAttributeCrawler($aElement, 'href');
                    if (Str::contains($href, 'brak.')){
                        return;
                    }
                    $main = sizeof($product->getImages()) === 0;
                    $href = explode('/s/', $href)[0];
                    $url = sprintf('https://b2b.ama-europe.pl%s', $href);
                    $idImage = explode('idZdjecie/', $url)[1];
                    $idImage = explode('/', $idImage)[0];
                    $filenameUnique = sprintf('%s.jpg', $idImage);
                    $id = $filenameUnique;
    ...
    {
        $crawlerDescription = $crawlerProduct->filter('div.productInfoDescription');
    ...
    {
        $nettoPrice = (float) $this->getAttributeCrawler($crawlerProduct->filter(sprintf('#price1n-%s', $product->getId())), 'value');
        $bruttoPrice = (float) $this->getAttributeCrawler($crawlerProduct->filter(sprintf('#price1b-%s', $product->getId())), 'value');
        $tax = (int) round((($bruttoPrice/$nettoPrice) - 1)* 100);
        if ($tax === null) {
